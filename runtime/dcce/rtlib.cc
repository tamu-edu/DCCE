...
#define MAX_THREADS (8)
  ...
  void addWeightEntry(uint64_t weight, uint64_t nid)
  {
    addWeight(weight, nid);
    entry_stack.push_back(std::make_pair(ccid, nid));
  }

  void removeWeightEntry(uint64_t weight, uint64_t nid)
  {
  ...
  void addWeightEB(uint64_t weight, uint64_t nid){
    int flag = 1;
    unsigned long top = entry_stack.size()-1;
    for(int i = top; i >= 0; i--){
      if(entry_stack[i].second == nid){
  ...
  void removeWeightEB(uint64_t weight, uint64_t nid)
  {
    unsigned long top = entry_stack.size()-1;
  ...
  uint64_t profile(uint64_t funcEntry/*same as nid*/)
  {
#ifdef DEBUG
  ...
  {
    pthread_mutex_lock(&lock_profile);
    std::string profile_file = "output/static_instrument/experiment/dcce/profile";
    switch (bench_code) {
