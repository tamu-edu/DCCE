...
#define MAX_THREADS (8)
  std::map<pthread_t, uint64_t> ccids;
  pthread_mutex_t lock_ccids;
  ...
  pthread_t getCurrentThreadID() {
    pthread_mutex_lock(&lock_ccids);
    pthread_t id = pthread_self();
    if (ccids.find(id) == ccids.end()) {
#ifdef DEBUG
      printf("New thread found id: %lu\n", id);
#endif
      ccids[id] = 0;
    }
    pthread_mutex_unlock(&lock_ccids);
    return id;
  }

  void initRuntime(unsigned int bench_code) {
    if (pthread_mutex_init(&lock_ccids, NULL) != 0) {
      printf("mutex init for lock_ccids has failed\n");
      exit(1);
    }
    ...
    // initialize ccid for main thread.
...
#endif
#ifdef DEBUG
    // ccid + weight <= ULONG_MAX
    if (ccids[id] > ULONG_MAX - weight) {
      printf("[tid:%lu][nid:%lu] CCID Overflow: CCID: %lu, weight: %lu\n", id, nid, ccids[id], weight);
      exit(1);
    }
#else
    assert(ccids[id] <= ULONG_MAX - weight);
#endif

#ifdef VERBOSE
    printf("[tid:%lu][nid:%lu] Before callsite, %lu + %lu => %lu\n", id, nid, ccids[id], weight, ccids[id] + weight);
#endif
    ccids[id] += weight;
    pthread_mutex_unlock(&lock_ccids);
  }
