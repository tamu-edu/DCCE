...
namespace SVF
{
    ...
    // DCCE related options
    const llvm::cl::opt<std::string> Options::dumpModules(
        "dump-modules",
        llvm::cl::value_desc("file path to dump"),
        llvm::cl::desc("Dump modules")
    );

    const llvm::cl::opt<std::string> Options::CallGraphDotGraph(
        "dump-callgraph",
        llvm::cl::value_desc("output file of callgraph"),
        llvm::cl::desc("Dump dot graph of Call Graph")
    );

    const llvm::cl::opt<std::string> Options::CCInput(
        "ccinput",
        llvm::cl::value_desc("calling context file"),
