...
MachineInstr *MachineFunction::CreateMachineInstr(const MCInstrDesc &MCID,
                                                  const DebugLoc &DL,
                                                  bool NoImplicit, const Instruction* Inst) {
  MachineInstr* MI = new (InstructionRecycler.Allocate<MachineInstr>(Allocator))
      MachineInstr(*this, MCID, DL, NoImplicit);
    if (Inst) {
        const CallBase* CI = dyn_cast_or_null<CallBase>(Inst);
        if (CI != NULL) {
          ...
          MI->setCCWeight(CCWeight);
          LLVM_DEBUG(dbgs() << "[danguria] MachineFunction::CreateMachineInstr " << *CI << " Adding CCWeight: " << CCWeight << "\n");
        }
    }
  return MI;
}
...
MachineFunction::CloneMachineInstr(const MachineInstr *Orig) {
  ...
  LLVM_DEBUG(dbgs() << "[danguria] MachineFunction::CreateMachineInstr " << *MI  << "\n");
  return MI;
}
